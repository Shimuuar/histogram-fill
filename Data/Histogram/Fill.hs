...
hbuilderTree fs h = HBuilder { hbInput  = \x -> mapM_ (flip hbInput x) hs
                             , hbOutput = sequence $ map hbOutput hs
                             }
    where hs = map ($ h) fs
